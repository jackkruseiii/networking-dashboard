...
const THRESHOLD = 90;
const AV = { cold:{bg:"#E6F1FB",color:"#0C447C"}, overdue:{bg:"#FAEEDA",color:"#633806"}, active:{bg:"#EAF3DE",color:"#3B6D11"} };
const COL = { cold:"#185FA5", overdue:"#854F0B", active:"#3B6D11" };
const BADGE = { never:{background:"#f5f5f3",border:"1px solid #ddd",color:"#999"}, overdue:{background:"#FAEEDA",border:"1px solid #EF9F27",color:"#854F0B"}, recent:{background:"#EAF3DE",border:"1px solid #97C459",color:"#3B6D11"} };
...
function lcCls(d,type){if(!d)return"never";if(type==="overdue")return"overdue";if(type==="cold")return"never";return ds(d)>=THRESHOLD?"overdue":"recent";}
...
function LastContactBadge({c,type}){
  ...
  return<div style={{display:"flex",alignItems:"center",gap:5,fontSize:11,padding:"5px 9px",borderRadius:7,...BADGE[lcCls(d,type)],width:"fit-content",marginBottom:9}}>
    <span style={{fontWeight:500}}>{fd(d)}</span><span style={{opacity:.75}}>— {label}</span>
  </div>;
}
...
function ContactCard({c,idx,type,onOpen,sessionNotes,setSessionNotes}){
  ...
  return<div style={{background:"#fff",border:"0.5px solid #e0e0de",borderRadius:12,padding:"14px 16px",marginBottom:10}}
    ...
    <div style={{display:"flex",alignItems:"flex-start",gap:10,marginBottom:9,cursor:"pointer"}} onClick={()=>onOpen(c,type)}>
      <div style={{width:34,height:34,minWidth:34,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:600,...av}}>{ini(c)}</div>
    ...
    {(c.industry||c.company||loc)&&<div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:9}}>
    ...
    {c.notes&&<div style={{fontSize:11,color:"#666",marginBottom:8,padding:"5px 8px",background:"#f9f9f7",borderRadius:6,borderLeft:"2px solid #ddd",lineHeight:1.5}}>
      <div style={{fontSize:10,color:"#999",marginBottom:2}}>Prior note</div>{c.notes}
    </div>}
    <textarea value={note} onChange={e=>{setSessionNotes(p=>({...p,[key]:e.target.value}));setShowSave(true);}}
    ...
    {showSave&&<button onClick={()=>{setSaved(true);setShowSave(false);setTimeout(()=>setSaved(false),2000);}}
      style={{marginTop:5,fontSize:11,padding:"3px 10px",borderRadius:6,border:"0.5px solid #ccc",background:"transparent",color:"#555",cursor:"pointer"}}>Save note</button>}
    {saved&&<div style={{fontSize:10,color:"#999",marginTop:3}}>Saved</div>}
  </div>;
}
...
function DetailPanel({c,type,onClose,sessionNotes,setSessionNotes}){
  ...
  function II({label,value}){return<div style={{background:"#f9f9f7",borderRadius:8,padding:"8px 10px"}}>
  ...
  return<div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.4)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
    <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:16,border:"0.5px solid #e0e0de",width:"min(560px,100%)",maxHeight:"85vh",overflowY:"auto",padding:24}}>
      <div style={{display:"flex",alignItems:"flex-start",gap:14,marginBottom:18}}>
        <div style={{width:52,height:52,minWidth:52,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,fontWeight:600,...av}}>{ini(c)}</div>
        <div style={{flex:1}}><div style={{fontSize:20,fontWeight:600,marginBottom:3}}>{c.fn} {c.ln}</div><div style={{fontSize:13,color:"#777"}}>{c.rel||(c.company||"—")}</div></div>
      ...
      {(c.linkedin||c.email)&&<div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
      ...
      <div style={{marginBottom:18}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          <II label="Status" value={c.status}/><II label="Industry" value={c.industry}/>
          <II label="Company" value={c.company}/><II label="Location" value={loc}/>
          <II label="Undergrad" value={c.ug}/><II label="Grad school" value={c.grad}/>
          <II label="Last check-in" value={d?fd(d):""}/><II label="Next check-in" value={nd?fd(nd):""}/>
        </div>
      </div>
      {c.notes&&<div style={{marginBottom:18}}>
      ...
      <div>
        ...
        <textarea value={note} onChange={e=>setSessionNotes(p=>({...p,[noteKey]:e.target.value}))} placeholder="Add a note about this interaction…" rows={3}
        ...
        <button onClick={()=>{setSaved(true);setTimeout(()=>setSaved(false),2000);}}
          style={{marginTop:6,fontSize:12,padding:"5px 14px",borderRadius:7,border:"0.5px solid #ccc",background:"transparent",color:"#555",cursor:"pointer"}}>Save note</button>
        {saved&&<span style={{fontSize:11,color:"#999",marginLeft:8}}>Saved</span>}
      </div>
    </div>
  </div>;
}
...
function NewContactModal({onClose,onAdd}){
  const [form,setForm]=useState({fn:"",ln:"",company:"",industry:"",rel:"",status:"Never Contacted",city:"",state:"",linkedin:"",email:"",ug:"",grad:"",lc:"",nc:"",notes:""});
  ...
  function submit(){
    ...
    onAdd({...form,fn:form.fn.trim(),ln:form.ln.trim()});
    onClose();
  }
  ...
  function Field({label,k,type="text",placeholder=""}){return<div style={{display:"flex",flexDirection:"column"}}>
    <label style={lbl}>{label}</label>
    <input type={type} value={form[k]} onChange={e=>set(k,e.target.value)} placeholder={placeholder} style={{...inp,borderColor:errors[k]?"#E24B4A":"#e0e0de"}}/>
  ...
  return<div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.4)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
    <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:16,border:"0.5px solid #e0e0de",width:"min(580px,100%)",maxHeight:"90vh",overflowY:"auto",padding:24}}>
      ...
      <div style={{display:"flex",gap:10,justifyContent:"flex-end",paddingTop:12,borderTop:"0.5px solid #eee"}}>
        ...
        <button onClick={submit} style={{fontSize:13,fontWeight:500,padding:"7px 18px",borderRadius:8,border:"none",background:"#1a1a18",color:"#fff",cursor:"pointer"}}>Add contact</button>
      </div>
    </div>
  </div>;
}
...
export default function NetworkingDashboard(){
  ...
  const {cold,overdue,active}=useMemo(()=>{
    ...
    const filtered=q?contacts.filter(c=>[c.fn,c.ln,c.company,c.industry,c.rel,c.city,c.state,c.notes].join(" ").toLowerCase().includes(q)):contacts;
    const cold=filtered.filter(c=>c.status==="Never Contacted");
    const all=filtered.filter(c=>c.status==="Active");
    const overdue=all.filter(c=>{const d=pd(c.lc);return d&&ds(d)>=THRESHOLD;}).sort((a,b)=>new Date(a.lc)-new Date(b.lc));
    const active=all.filter(c=>{const d=pd(c.lc);return !d||ds(d)<THRESHOLD;}).sort((a,b)=>new Date(b.lc)-new Date(a.lc));
    return{cold,overdue,active};
  },[contacts,query]);

  const columns=[
    {key:"cold",title:"Cold Outreach",contacts:cold},
    {key:"overdue",title:"Overdue",contacts:overdue},
    {key:"active",title:"Active",contacts:active},
  ];
  const icons={cold:"✉️",overdue:"⏰",active:"✅"};

  return<div style={{fontFamily:"Georgia,serif",background:"#fafaf8",minHeight:"100vh",paddingBottom:"3rem"}}>
    <div style={{background:"#fff",borderBottom:"0.5px solid #e8e8e4",padding:"16px 24px",display:"flex",alignItems:"center",gap:12,flexWrap:"wrap",marginBottom:20}}>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
        {columns.map(col=><div key={col.key} style={{fontSize:12,padding:"4px 11px",borderRadius:20,fontWeight:600,...(col.key==="cold"?{background:"#E6F1FB",color:"#185FA5"}:col.key==="overdue"?{background:"#FAEEDA",color:"#854F0B"}:{background:"#EAF3DE",color:"#3B6D11"})}}>
    ...
    {query&&<div style={{padding:"0 24px 12px",fontSize:12,color:"#999"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(3,minmax(0,1fr))",gap:20,padding:"0 24px"}}>
      {columns.map(col=><div key={col.key} style={{minWidth:0}}>
        ...
        {col.contacts.length===0
          ...
          :col.contacts.map((c,i)=><ContactCard key={`${col.key}-${c.fn}-${c.ln}-${i}`} c={c} idx={i} type={col.key}
    ...
    {selected&&<DetailPanel c={selected} type={selectedType} onClose={()=>setSelected(null)} sessionNotes={sessionNotes} setSessionNotes={setSessionNotes}/>}
    {showNew&&<NewContactModal onClose={()=>setShowNew(false)} onAdd={c=>setContacts(p=>[...p,c])}/>}
  </div>;
}
